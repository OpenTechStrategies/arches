...
const MapFilterViewModel = function (params) {
    var self = this;
    ...
    this.layers = ko.observable([
        {
            ...
            "paint": { "line-color": "#aa0000", "line-width": 4 }
        },
        {
            ...
            "filter": ["==", "$type", "Polygon"],
            "layout": { "line-cap": "round", "line-join": "round" },
            "paint": { "line-color": "#aa0000", "line-width": 2 }
        },
        {
        ...
        },
        {
            ...
            "paint": { "fill-color": "#aa3322", "fill-outline-color": "#aa0000", "fill-opacity": 0.2 },
            "source": "geojson-search-buffer-data"
        },
        {
        ...
        },
        {
            ...
            "type": "circle",
            "filter": ["==", "$type", "Point"],
            "paint": { "circle-radius": 5, "circle-color": "#aa0000" },
            "source": "geojson-search-buffer-data"
        }
    ]);

    this.clear = function () {
        this.map().getSource('geojson-search-buffer-data').setData({ "type": "FeatureCollection", "features": [] });
    ...
    this.spatialFilterTypes = [
        { name: 'Point', title: 'Draw a Marker', class: 'leaflet-draw-draw-marker', icon: 'ion-location', drawMode: 'draw_point', active: ko.observable(false) },
        { name: 'Line', title: 'Draw a Polyline', icon: 'ion-steam', class: 'leaflet-draw-draw-polyline', drawMode: 'draw_line_string', active: ko.observable(false) },
        { name: 'Polygon', title: 'Draw a Polygon', icon: 'fa fa-pencil-square-o', class: 'leaflet-draw-draw-polygon', drawMode: 'draw_polygon', active: ko.observable(false) }
    ];
    ...
    this.setupDraw = function () {
        require(['mapbox-gl-draw'], (MapboxDraw) => {
            ...
            if (!this.draw) {
                ...
                this.draw = new MapboxDraw({ displayControlsDefault: false, modes: modes });
                this.map().addControl(this.draw);
            }
            this.map().on('draw.create', function (e) {
                if (self.searchContext()) {
                    self.draw.getAll().features.forEach(function (feature) {
                        if (feature.id !== e.features[0].id && feature.properties.searchGeom) {
                            self.draw.delete(feature.id);
                        } else {
    ...
    this.updateFilter = function () {
        ...
        this.searchGeometries().forEach(function (feature) {
            ...
            feature.properties.buffer = { "width": this.buffer(), "unit": this.bufferUnit() };
            feature.properties.inverted = this.filter.inverted();
        }, this);
        this.filter.feature_collection({ "type": "FeatureCollection", "features": this.searchGeometries() });
    };
