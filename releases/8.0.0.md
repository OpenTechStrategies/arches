...
1. Update `.gitignore`:
    - Remove `.tsconfig-paths.json` and `.frontend-configuration-settings.json`
    - Add `frontend_configuration`
...
1. (optional) Remove `templates/arches_urls.htm`:
    - This change will be enforced in later Arches versions.
    - Instead of accessing urls via `import arches from "arches";`, you can instead use the `generateArchesUrl` utility.
        ...
        const url = `${arches.urls.my_awesome_url}?my_awesome_query_arg='foo'`
        fetch(url).then(resp => {...});
        ```
        ...
        const url = generateArchesURL("my_awesome_url", { my_awesome_query_arg: "foo" });
        fetch(url).then(resp => {...});
        ```
